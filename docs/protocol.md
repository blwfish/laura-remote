...
Three roles, two RX sub-variants:

| Role | PCB | Pigtail | Notes |
| --- | --- | --- | --- |
| TX | TX PCB | none | Handheld transmitter. GPS, OLED, buttons, LEDs. |
| RX-10pin | RX PCB | Nikon round 10-pin | Z9, Z8, D5 etc. Ready-pin wired to MCU input; fire confirmation supported. |
| RX-MCDC2 | RX PCB | Nikon MC-DC2 (rectangular 10-pin) | Z5, Z6/Z6II/Z6III, Z7/Z7II etc. No Ready-pin signal available; ACKs use `OK_CMD_SENT` outcome. Port supplies +5 V which can optionally power the RX parasitically. |
| Repeater | RX PCB | none (or SMA-only chassis) | Same PCB as RX. Output stage unpopulated. |

The RX PCB is a single design serving both pigtail variants; the difference is purely which pins on a common board-side connector are brought out to the pigtail. Firmware detects the variant via a strap resistor on the PCB-side connector (presence/absence of a Ready-pin line) and gates Ready-pin-dependent logic accordingly.
...
| Code | Meaning |
| --- | --- |
| 0x00 | `OK_FIRED` — shutter asserted and Ready-pin transition observed (10-pin variant only) |
| 0x01 | `OK_KEEPALIVE_STARTED` — keep-alive armed, first pulse sent |
| 0x02 | `OK_KEEPALIVE_STOPPED` |
| 0x03 | `OK_PING` |
| 0x04 | `OK_CMD_SENT` — trigger contact asserted; RX has no Ready-pin feedback to independently confirm fire (MC-DC2 variant on supported bodies) |
| 0x10 | `WARN_CMD_SENT_NO_FIRE_DETECTED` — 10-pin variant only; we poked the pin but didn't see a Ready-pin transition (rare; long exposure or camera anomaly) |
| 0x20 | `ERR_NOT_READY` — 10-pin variant only; Ready pin was low before command, so camera was not ready |
| 0x21 | `ERR_CAMERA_UNRESPONSIVE` — 10-pin variant only; keep-alive pulse issued but Ready pin never rose |
| 0x30 | `ERR_BAD_COMMAND` |
| 0x31 | `ERR_BAD_MIC` |

The MC-DC2 variant cannot observe Ready-pin transitions because the MC-DC2 port does not expose such a signal. A successful `SHOOT` on an MC-DC2 RX returns `OK_CMD_SENT`; the `OK_FIRED` / `ERR_NOT_READY` / `ERR_CAMERA_UNRESPONSIVE` / `WARN_*` codes are never generated by MC-DC2 RXs. The TX UI maps `OK_CMD_SENT` to amber on the Fire LED (distinguishable from the green of `OK_FIRED`) so the operator can see at a glance which class of confirmation was achieved.
...
| 3 | Ready-pin observations | 10-pin variant: bit 0 = Ready at time of cmd, bit 1 = transition observed this cmd, bits 2–6 reserved, bit 7 = 0. MC-DC2 variant: bit 7 = 1 (indicates "Ready-pin not available on this hardware"), other bits undefined. |
